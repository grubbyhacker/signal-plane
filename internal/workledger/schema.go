...
func Migrate(ctx context.Context, db *sql.DB) error {
	...
	statements := []string{
		...
		`CREATE TABLE IF NOT EXISTS repository_ci_tasks (job_id INTEGER PRIMARY KEY REFERENCES jobs(id), repository TEXT NOT NULL, issue_number INTEGER NOT NULL, pull_number INTEGER NOT NULL CHECK(pull_number>0), branch TEXT NOT NULL, current_head_sha TEXT NOT NULL, agent_model TEXT NOT NULL, state TEXT NOT NULL CHECK(state IN ('waiting_ci','repair_ready','repair_running','completed','escalation_pending','escalation_blocked','escalated')), repair_attempts INTEGER NOT NULL DEFAULT 0 CHECK(repair_attempts>=0 AND repair_attempts<=2), deadline_at INTEGER NOT NULL, initial_result_json TEXT NOT NULL, last_ci_state TEXT NOT NULL DEFAULT '', last_ci_json TEXT NOT NULL DEFAULT '', created_at INTEGER NOT NULL, updated_at INTEGER NOT NULL, terminal_at INTEGER)`,
		`CREATE INDEX IF NOT EXISTS repository_ci_tasks_head ON repository_ci_tasks(repository,current_head_sha,state)`,
		`CREATE TABLE IF NOT EXISTS repository_ci_events (delivery_id TEXT PRIMARY KEY, repository TEXT NOT NULL, pull_number INTEGER NOT NULL DEFAULT 0, head_sha TEXT NOT NULL, event_kind TEXT NOT NULL, action TEXT NOT NULL, semantic_identity TEXT NOT NULL, payload_digest TEXT NOT NULL, received_at INTEGER NOT NULL, recorded_at INTEGER NOT NULL)`,
	...
	for _, column := range []struct{ name, definition string }{
		...
		// Agent-platform-coupling Stage 5 foundation. Additive, zero-value safe.
		// These are platform/broker-authoritative; emitters and runtime callers
		// never supply them and never select an image, release, or generation.
		{"agent_type", `agent_type TEXT NOT NULL DEFAULT ''`},
		{"agent_mode", `agent_mode TEXT NOT NULL DEFAULT ''`},
		{"type_contract_revision", `type_contract_revision TEXT NOT NULL DEFAULT ''`},
		{"resolved_release_generation", `resolved_release_generation INTEGER NOT NULL DEFAULT 0 CHECK(resolved_release_generation>=0)`},
		{"resolved_release_digest", `resolved_release_digest TEXT NOT NULL DEFAULT ''`},
		{"broker_run_id", `broker_run_id TEXT NOT NULL DEFAULT ''`},
		{"authoritative_pr_repository", `authoritative_pr_repository TEXT NOT NULL DEFAULT ''`},
		{"authoritative_pr_number", `authoritative_pr_number INTEGER NOT NULL DEFAULT 0 CHECK(authoritative_pr_number>=0)`},
	} {
		if err := ensureMigrationColumn(ctx, tx, "work_items", column.name, column.definition); err != nil {
			return err
		}
	}
	// These indexes cover the agent columns and must run after the columns are
	// ensured, so an upgraded pre-agent work_items table has them present.
	for _, index := range []string{
		`CREATE INDEX IF NOT EXISTS work_items_agent_type ON work_items(agent_type,agent_mode,created_at)`,
		`CREATE INDEX IF NOT EXISTS work_items_authoritative_pr ON work_items(authoritative_pr_repository,authoritative_pr_number)`,
	} {
		if _, err := tx.ExecContext(ctx, index); err != nil {
			return fmt.Errorf("migrate work ledger agent index: %w", err)
		}
	}
	for _, column := range []struct{ name, definition string }{
