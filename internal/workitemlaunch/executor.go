...
type Broker interface {
	LaunchWorkItem(ctx context.Context, profile, workItemID, uploadID, idempotencyKey string) (dispatcher.LaunchResult, error)
}
...
func (executor *Executor) Execute(ctx context.Context, request workledger.ExecutorRequest) (workledger.ExecutorResult, error) {
	...
	if item.ObjectKind != "upload" || item.ObjectID == "" {
		return permanent("work item is not an authoritative upload"), nil
	}
	...
	result, err := executor.Broker.LaunchWorkItem(ctx, profile, item.ID, item.ObjectID, request.Attempt.IdempotencyKey)
	if err != nil {
