...
RUN CGO_ENABLED=0 GOOS=linux go build -o /out/signal-gateway ./cmd/signal-gateway \
 ...
 && CGO_ENABLED=0 GOOS=linux go build -o /out/github-green-pr-coordinator ./cmd/github-green-pr-coordinator \
 && CGO_ENABLED=0 GOOS=linux go build -o /out/github-green-pr-fixture-admit ./cmd/github-green-pr-fixture-admit \
...
COPY --from=build /out/github-green-pr-coordinator /usr/local/bin/github-green-pr-coordinator
COPY --from=build /out/github-green-pr-fixture-admit /usr/local/bin/github-green-pr-fixture-admit
