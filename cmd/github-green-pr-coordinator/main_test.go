...
func (b *fixtureBroker) SubmitTurn(_ context.Context, request agentsession.SubmitTurnRequest) (agentsession.BrokerTurn, error) {
	...
	return agentsession.BrokerTurn{SessionID: "fixture-session", TurnID: "fixture-turn", ModelEffectID: "model:" + request.IdempotencyKey, SubmitCursor: 1, Lease: b.lease}, nil
}
...
func TestFixtureAdmissionRunsSubmitThenScheduledPollWithoutAnotherTurn(t *testing.T) {
	...
	if len(broker.turns) != 1 || len(broker.streams) != 3 || broker.streams[0].Cursor != 0 || broker.streams[1].Cursor != 0 || broker.streams[2].Cursor != 1 {
	...
	if len(broker.turns) != 1 || len(broker.streams) != 4 || broker.streams[3].Cursor != 2 {
